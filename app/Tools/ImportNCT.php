...
//        dd($this->_xmlLink);
	...
	{
		$items = $crawler->filterXPath('//tracklist/track')->each(function (Crawler $node, $i) {
			return array(
				'title' => $node->filterXPath('//title')->text(),
				'performer' => $node->filterXPath('//creator')->text(),
				'source' => $node->filterXPath('//location')->text(),
				'lyric' => $node->filterXPath('//lyric')->text(),
//				'lyric' => file_get_contents($node->filterXPath('//lyric')->text()),
				'backimage' => $node->filterXPath('//bgimage')->text(),
				'playlist_img' => $node->filterXPath('//avatar')->text()
			);
		});
		return $items;
	}
